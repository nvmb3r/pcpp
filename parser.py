...
import sys, argparse, traceback, os, copy, io, re, json
...
class CmdPreprocessor(Preprocessor):
    def __init__(self, argv):
        ...
        argp.add_argument('inputs', metavar='input', default=[sys.stdin], nargs='*', action=FileAction,
                          help='Files to preprocess (use \'-\' for stdin)')
        argp.add_argument('-o', dest='output', metavar='path', type=argparse.FileType('wt'), default=sys.stdout,
                          nargs='?', help='Output to a file instead of stdout')
        argp.add_argument('-D', dest='defines', metavar='macro[=val]', nargs=1, action='append',
                          help='Predefine name as a macro [with value]')
        argp.add_argument('-U', dest='undefines', metavar='macro', nargs=1, action='append',
                          help='Pre-undefine name as a macro')
        argp.add_argument('-N', dest='nevers', metavar='macro', nargs=1, action='append',
                          help='Never define name as a macro, even if defined during the preprocessing.')
        argp.add_argument('-I', dest='includes', metavar='path', nargs=1, action='append',
        ...
        argp.add_argument('--passthru-defines', dest='passthru_defines', action='store_true',
                          help='Pass through but still execute #defines and #undefs if not always removed by preprocessor logic')
        argp.add_argument('--passthru-unfound-includes', dest='passthru_unfound_includes', action='store_true',
                          help='Pass through #includes not found without execution')
        argp.add_argument('--passthru-unknown-exprs', dest='passthru_undefined_exprs', action='store_true',
        ...
        argp.add_argument('--passthru-comments', dest='passthru_comments', action='store_true',
                          help='Pass through comments unmodified')
        argp.add_argument('--passthru-magic-macros', dest='passthru_magic_macros', action='store_true',
                          help='Pass through double underscore magic macros unmodified')
        argp.add_argument('--passthru-includes', dest='passthru_includes', metavar='<regex>', default=None, nargs=1,
                          help="Regular expression for which #includes to not expand. #includes, if found, are always executed")
        argp.add_argument('--disable-auto-pragma-once', dest='auto_pragma_once_disabled', action='store_true',
                          default=False,
        ...
        argp.add_argument('--line-directive', dest='line_directive', metavar='form', default='#line', nargs='?',
        ...
        argp.add_argument('--time', dest='time', action='store_true',
                          help='Print the time it took to #include each file')
        argp.add_argument('--filetimes', dest='filetimes', metavar='path', type=argparse.FileType('wt'), default=None,
                          nargs='?',
        ...
        argp.add_argument('--assume-input-encoding', dest='assume_input_encoding', metavar='<encoding>', default=None,
                          nargs=1, help='The text encoding to assume inputs are in')
        argp.add_argument('--output-encoding', dest='output_encoding', metavar='<encoding>', default=None, nargs=1,
                          help='The text encoding to use when writing files')
        argp.add_argument('--write-bom', dest='write_bom', action='store_true',
        ...
        if self.args.time:
            ...
            for n in range(0, len(self.include_times)):
                if n == 0:
                    ...
                    print("\n %s: %f seconds (%f%%)" % (
                        self.include_times[n].included_path, self.include_times[n].elapsed,
                        100 * self.include_times[n].elapsed / self.include_times[0].elapsed))
                else:
                    print("%s%s: %f seconds" % (' ' * self.include_times[n].depth, self.include_times[n].included_path,
    ...
    def on_include_not_found(self, is_malformed, is_system_include, curdir, includepath):
        ### ifdefuzz
        global directives
        _file = {'in_file:': includepath,
                 'is_system_include': is_system_include,
                 'is_found:': False
                 }
        directives[len(directives)] = _file
        ###
        ...
        if toks:
            ...
            for tok in toks:
                _tok = {'id': tok.type,
                        ...
                        'line': tok.lineno}
                _toks.append(_tok)
            _directive = {'id': directive.type + '_DIRECTIVE',
                          'value': directive.value,
                          'line': directive.lineno}
            directives[len(directives)] = [_directive] + [_toks]
        else:
            _directive = {'id': directive.type + '_DIRECTIVE',
                          'value': directive.value,
                          'line': directive.lineno}

            directives[len(directives)] = [_directive]
        ###########################
        if ifpassthru:
...
if __name__ == "__main__":
    p = CmdPreprocessor(sys.argv)
    # for d in directives:
    #     print(d, directives[d])
    with open(sys.argv[-1] + '.parsed', "w+") as myfile:
        myfile.write(json.dumps(directives))
    sys.exit(p.return_code)